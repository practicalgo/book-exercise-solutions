...
func packageQueryHandler(w http.ResponseWriter, r *http.Request, config appConfig) {
	...
	if len(owner) == 0 && len(name) == 0 && len(version) != 0 {
		http.Error(
			w,
			"Must specify package owner or name when version is specified",
			http.StatusBadRequest,
		)
		return
	}
	...
	resp := pkgQueryResponse{
		Packages: pkgResults,
	}
	jsonPkgResults, err := json.Marshal(resp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	w.Header().Add("Content-Type", "application/json")
	fmt.Fprint(w, string(jsonPkgResults))
}

func packageDownloadHandle(w http.ResponseWriter, r *http.Request, config appConfig) {
	queryParams := r.URL.Query()
	owner := queryParams.Get("owner_id")
	name := queryParams.Get("name")
	version := queryParams.Get("version")
...
func packageDownloadHandler(
	...
	switch r.Method {
	case "GET":
		packageDownloadHandle(w, r, config)
	default:
		http.Error(w,
			"Cannot process request",
			http.StatusMethodNotAllowed,
